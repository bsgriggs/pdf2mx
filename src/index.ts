...
async function main(data: IInputModel, params: IParams) {
    ...
    if (data.attributes.length != cleanedInput.attributes.length) {
        console.warn(
            `Removed ${data.attributes.length - cleanedInput.attributes.length
            } duplicate attributes`
        );
    }

    const {model, workingCopy} = await connectToModel(params);
    console.debug('entityName: ' + params.entityName)
    console.debug('moduleName: ' + params.moduleName)
    await createEntity(cleanedInput, params, model, workingCopy);
    await createPage(cleanedInput, params, model, workingCopy);
    await commit(workingCopy, model, `Add Entity ${params.moduleName}.${params.entityName} and page.`, params.branch || BRANCH as string);
